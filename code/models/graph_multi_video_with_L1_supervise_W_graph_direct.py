...
class Graph_Multi_Video(nn.Module):
    def __init__(self,
        ...
        for tp in to_pend:
            if tp.lower()=='ht':
                ...
                self.feature.append(nn.Dropout(0.5))
            else:
        ...
        self.graph_layer = Graph_Layer_Wrapper(in_out[0],n_out = n_classes, non_lin = non_lin, method = method, aft_nonlin = aft_nonlin)
    ...
    def forward(self, input, epoch_num = None, ret_bg =False, branch_to_test = -1):
        ...
        for input in input_chunks:
            ...
            for idx_sample in range(len(input_sizes)):
                ...
                pred_graph = out_graph[start:end,:]

                # pred_graph = self.last_graph(out_graph_curr)
                pred_linear = self.last_feat(inter_out_curr)
                for idx_x_curr, x_curr in enumerate([pred_linear,pred_graph]):

                    x_all_all[idx_x_curr].append(x_curr)
                    pmf_all[idx_x_curr].append(self.make_pmf(x_curr).unsqueeze(0))

                # x_all_all[1].append(pred_graph)
    ...
    def make_pmf(self,x):
        ...
        pmf = pmf[:k,:]
        pmf = torch.sum(pmf[:k,:], dim = 0)/float(k)
...
class Network:
    ...
    def get_lr_list(self, lr):
        ...
        modules = []
        modules+=[self.model.feature, self.model.linear_layer, self.model.graph_layer, self.model.last_feat]
